...
const Home = () => {
  ...
  const loadParking = async (form) => {
    ...
    setParkingLots((prevParkingLots) => [...prevParkingLots, ...data]); // Nối dữ liệu mới vào dữ liệu cũ
  };
  useEffect(() => {
    loadParking(form);
  }, [form]);
  ...
  return (
    ...
    >
      <div className="p-3" style={{ width: "50%" }}>
        ...
        <Button onClick={() => handlePayMomo(50000)}> Thanh toán Momo</Button>
        <Button onClick={() => handlePayPal(60000)}> Thanh toán PayPal</Button>
